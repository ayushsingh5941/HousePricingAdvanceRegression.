...
def feature_engg(data):
    ...
    if 'SalePrice' in data.columns:
        ...
        """
        data = data.drop(data[data.MSSubClass >130].index)

        # 1stflrsf
        data = data.drop(data[data['1stFlrSF'] > 3000].index)

        # GarageCars
        data = data.drop(data[data.GarageCars > 3].index)
       

        # Overall Qual
        data = data.drop(data[data.OverallQual < 2].index)
    ...
    data['TotalCarpetArea'] = data['TotalBsmtSF'] + data['1stFlrSF'] \
                              + data['2ndFlrSF'] + data['LowQualFinSF'] + data['GarageArea']
    ...
    data['TotalBathroom'] = data['BsmtFullBath'] + (0.5 * data['BsmtHalfBath']) + data['FullBath'] \
                            + (0.5 * data['HalfBath'])
...
# RMLSE regressor valid 0.12003093232201684
...
#RMLSE regressor valid 0.12010869136356805

# _________________________________RIDGE REGRESSION__________________________
ridge_reg = Ridge(random_state=42)
ridge_param = {'fit_intercept': [True],
               'alpha': [0.01],
               'normalize': [True],
               'tol': [1e-3]
               }
model_ridge = grid_search(ridge_reg, ridge_param)
# RMLSE regressor valid 0.12541978292777728



# ______________________________XGBOOST_____________________________
lgboost_reg = lgbm.LGBMRegressor()
xgb_param = {'learning_rate': [0.05],
             'num_leaves': [255],
             'boosting_type': ['gbdt', 'dart', 'goss']
             }
model_xgb = grid_search(lgboost_reg, xgb_param)
# RMLSE regressor valid 0.1378748181564448
...
# RMLSE regressor valid 0.12979852576706105

# ________________  CATBOOST___________________________
cat_boost = cat.CatBoostRegressor()
cat_param = {}
model_cat = grid_search(cat_boost, cat_param)


# ================================ _________________________STACKING ALGOS________________===================
estimators = [model_elastic, model_cat, model_xgb, model_svr]
...
"""
# ===============================____________________________NEURAL NETWORK____________________________________
print('___________________Neural Network_____________')
model = keras.models.Sequential()
model.add(keras.layers.Dense(30, activation='relu', input_shape=x_train.shape[1:]))
model.add(keras.layers.Dense(1))
model.compile(loss='mean_squared_error', optimizer='adam', metrics=['accuracy'])
early_stopping_cb = keras.callbacks.EarlyStopping(patience=10, restore_best_weights=True)
history = model.fit(x_train, y_train, batch_size=32, epochs=20, callbacks=[early_stopping_cb]
